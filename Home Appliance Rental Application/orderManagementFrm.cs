...
{
    public partial class orderManagementFrm : Form
    {
        public orderManagementFrm()
        {
            InitializeComponent();
        }
        ...
        public DataTable BindSource()
        {
            ...
            else
            {
                ...
                MessageBox.Show("No orders have been placed in this system yet", "Home Appliance Rental System", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            // Return datatable
            return dt;
        }

        private void orderManagementFrm_Load(object sender, EventArgs e)
        {
            BindSource();
        }
        ...
        private void orderNumberTxt_TextChanged(object sender, EventArgs e)
        {
            ...
            con.Open();
            // Retrieve all surveys from 'survey' table
            cmd = new SqlCommand("SELECT * FROM [order] WHERE order_id = " + orderNumberTxt, con);
            da = new SqlDataAdapter(cmd);
            // Clear existing dataset if any
            ds.Clear();
            // Store retrived surveys in dataset
        ...
        {
            orderID = Convert.ToInt32(myOrdersDgv.CurrentRow.Cells["ID"].Value);
            cmd = new SqlCommand("SELECT * FROM [order] WHERE Id = " + orderID, con);
            da = new SqlDataAdapter(cmd);
            ...
            updateOrderStatusFrm updateOrderStatusFrm = new updateOrderStatusFrm();
            updateOrderStatusFrm.ShowDialog();
        }
    }
}
